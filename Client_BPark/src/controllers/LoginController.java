...
public class LoginController implements Initializable {
	...
	public void handleLoginFailed(String reason) {
		Platform.runLater(() -> {
			String errorMessage = reason != null ? reason : "Login failed. Please check your credentials.";
	...
	public void handleLoginSuccess(String userType) {
		Platform.runLater(() -> {
			...
			CustomDialog.showInformation(
				"Login Successful",
				"Welcome!",
				"Login successful! Loading your personalized interface..."
			);
	...
	/**
	 * Shows the parking availability result in a custom dialog
	 * 
	 * @param availableSpots The number of available parking spots
	 */
	public void showAvailabilityResult(int availableSpots) {
		Platform.runLater(() -> {
			String message;
			String header;
			...
			if (availableSpots > 0) {
				...
				message = "There are currently " + availableSpots + " parking spots available.";
				
				if (availableSpots < 5) {
					message += "\n\n⚠️ Limited spots remaining - arrive soon!";
				}
			} else {
				header = "No Parking Available";
				message = "Sorry, there are currently no parking spots available.\nPlease try again later or contact support.";
			}
