...
public class SubscriberController implements Initializable {
	...
	private void handleMakeReservation() {
		...
		if (selectedDate == null || selectedTime == null) {
			CustomDialog.showError("Error", "Missing Information", "Please select both date and time");
			return;
		}
	...
	private void handleCancelReservationFromPage() {
		...
		if (code != null && !code.trim().isEmpty()) {
			...
			if (result == CustomDialog.RESULT_YES) {
				...
				if (userID == 0) {
					CustomDialog.showError("User Error", "Authentication Error", "Error: User not logged in properly.");
					return;
				}
			...
			CustomDialog.showError("Missing Code", "Input Required", "Please enter a reservation code before clicking Cancel.");
		}
	}
	...
	private void showError(String title, String content) {
		CustomDialog.showError(title, title, content);
	}
}